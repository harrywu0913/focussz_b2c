package com.focussz.common.core.web;
...
import com.focussz.common.core.biz.Services;
import com.focussz.common.page.PagerModel;

/**
 * 基础的action
 */
public abstract class BaseController<E extends PagerModel<T>, T> {
	 ...
	 * 分页模版
	 */
	protected PagerModel<T> pager = new PagerModel<T>();
	/**
	 * 抽象的业务逻辑层接口句柄，子类可以通过重写setServer方法注入指定的具体业务逻辑句柄来实现个性化需求
	 */
	protected Services<E,T> server;
	protected E e;
	 ...
	 * @return
	 */
	public Services<E,T> getServer() {
		return this.server;
	}

	public void setServer(Services<E,T> server) {
		this.server = server;
	}

	public PagerModel<T> getPager() {
		return pager;
	}

	public void setPager(PagerModel<T> pager) {
		this.pager = pager;
	}
	 ...
	 * @throws Exception
	 */
	public PagerModel<T> selectPagerModelByServices(Services<E, T> services, PagerModel<T> pm) throws Exception {
		...
		if (offset < 0)
			offset = 0;
		pm.setOffset(offset);
		PagerModel<T> servicesPager = services.selectPageList(pm);
		if (servicesPager == null)
			servicesPager = new PagerModel<T>();
		// 计算总页数
	...
	public String selectList() throws Exception {
		...
		if (offset < 0)
			offset = 0;
		((PagerModel<T>) getE()).setOffset(offset);
		pager = getServer().selectPageList(getE());
		if (pager == null)
			pager = new PagerModel<T>();
		// 计算总页数
	...
	public String deletes() throws Exception {
		...
		// throw new NullPointerException();
		// }
		// if(user.getDbPrivilegeMap()!=null &&
		// user.getDbPrivilegeMap().size()>0){
